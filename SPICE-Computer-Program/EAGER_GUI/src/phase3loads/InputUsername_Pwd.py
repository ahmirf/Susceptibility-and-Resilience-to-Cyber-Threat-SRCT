...
class EmailLinkInputUsernamePwd(CustomDialog.Dialog):
    def __init__(self, parent,title,parentEmailPojo,dialogID, dialogTag,startTime): 
#         print "===> form_username_ped.py initiatex"
        self.name = "Input username and password to hack in hacking email_1"
        self.type = 1
        self.startTime=startTime
        self.uName_pwd_correct=False
...
#         self.pwdStatus.Hide()
        ...
        self.Loginsizer.Add(userNamesizer,1)
        self.Loginsizer.Add(pwdsizer,1)
        self.Loginsizer.Add((5,9))
        self.Loginsizer.Add(btnSizer,0)
    ...
    def onClickOK(self,event):
        ...
        if result==True:
            self.uName_pwd_correct=False
            if not globalVar.dialogPojo_==None and (globalVar.dialogPojo_).getID()==self.dialogID and isinstance(globalVar.dialogPojo_, dialogMsgPojo):
                if len((globalVar.dialogPojo_).getUserName().strip())>0 and  len((globalVar.dialogPojo_).getPassword().strip())>0:
                    requiredUserName=(globalVar.dialogPojo_).getUserName().strip()
                    requiredPwd=(globalVar.dialogPojo_).getPassword().strip()
                    print "======> requiredUserName = "+requiredUserName+"\t requiredPwd= "+requiredPwd
                    if requiredUserName==userName and requiredPwd==pwd:
                        #then allow to get the hacking information "Information on the dialog 1009"
                        self.uName_pwd_correct=True
            ...
            if self.uName_pwd_correct:
                self.Hide()
            else:
                wx.MessageBox("UserName or Password you typed is incorrect", "Info", wx.ICON_INFORMATION)
...
class App(wx.App):
    ...
    def OnInit(self):
        ...
        frame = EmailLinkInputUsernamePwd(parent=None,title="Chance to win a tour to Facebook")
